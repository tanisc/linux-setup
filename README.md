...
| `dotfiles` | Copies generic config files/dirs; forces the credential files (`rclone.conf`, `.s3cfg`, `winapps.conf`) to 600 — the copies in `data/` on backup, the restored files on restore | `dotfiles.list` |
| `systemd_user` | Copies systemd user units in `~/.config/systemd/user/`; on restore, creates each unit's mount point (the `%h/<dir>` from its `ExecStartPre=... mkdir -p` line, e.g. `~/OneDrive`), then `daemon-reload` and `enable --now` each one (a unit that fails to start only warns). Runs after `dotfiles`, so `rclone.conf` is already in place for the rclone mounts | `systemd-user-units.list` |
...
yourself. `dotfiles`, `systemd_user`, `tools_scripts`, `conda_envs`, and `ssh_gpg` are symmetric
(`backup.sh` captures them too).
...
- **`systemd-user-units.list`**: rclone mounts of remotes defined in `rclone.conf`, each with a
  full VFS cache (max 5G), started at login:
  - `rclone-onedrive.service` — `onedrive_fmi:` → `~/OneDrive`
  - `rclone-gdrive.service` — `gdrive:` → `~/GoogleDrive`
  - `rclone-gdrive-shared.service` — `gdrive-shared:` ("Shared with me") → `~/GoogleDriveShared`
...
- **KDE session / `docker` group**: both require a reboot or re-login to actually take effect
  ...
  reboot for you.
- **rclone mounts (OneDrive / Google Drive)**: need their remotes (`onedrive_fmi`, `gdrive`,
  `gdrive-shared`) in the backed-up `rclone.conf`, so run `backup.sh` after setting up a remote.
  If a remote's OAuth token has expired by the time you restore, its mount fails to start;
  refresh it with `rclone config reconnect <remote>:`, then
...
Considered and explicitly declined (not needed for this setup): fonts, crontab,
`/usr/local/bin` scripts, dconf/keyboard shortcuts/GNOME extensions, NetworkManager saved
connections, `~/Downloads` installer leftovers.
