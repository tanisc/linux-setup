...
| `tools_installers` | Apps under `~/.tools` needing their own installer, target dir comes from the manifest | `tools-installers.list` |
...
| `ssh_gpg` | Copies `~/.ssh`, fixes permissions (700 dir / 600 keys / 644 `*.pub`) | (whole-directory copy, no manifest) |
...
yourself. `dotfiles`, `tools_scripts`, and `ssh_gpg` are symmetric (`backup.sh` captures them
too).
...
saved connections, `~/Downloads` installer leftovers, GPG keys (none exist on this machine).
